...
#####
# WAFv2 web acl association with AWS resource
#####
resource "aws_wafv2_web_acl_association" "main" {
  count = var.enabled && var.create_aws_resource_association && length(var.association_arn_list) == 0 ? 1 : 0
...
resource "aws_wafv2_web_acl_association" "association_list" {
  count = var.enabled && var.create_aws_resource_association && length(var.association_arn_list) > 0 ? length(var.association_arn_list) : 0

  resource_arn = var.association_arn_list[count.index]
  web_acl_arn  = aws_wafv2_web_acl.main[0].arn
